function UnityProgress (dom) {
	...
	background.style.backgroundImage = 'url(WebContent/cover.png)';
	background.style.backgroundSize = this.dom.offsetWidth + 'px' + ' ' + this.dom.offsetHeight + 'px';
	background.style.position = 'absolute';
	parent.appendChild(background);
	this.background = background;

	var logoImage = document.createElement('img');
	logoImage.src = 'WebContent/progresslogo.png'; 
	logoImage.style.position = 'absolute';
	parent.appendChild(logoImage);
	this.logoImage = logoImage;

	var progressFrame = document.createElement('img');
	progressFrame.src = 'WebContent/loadingbar.png'; 
	progressFrame.style.position = 'absolute';
	parent.appendChild(progressFrame);
	this.progressFrame = progressFrame;

	var progressBar = document.createElement('img');
	progressBar.src = 'WebContent/fullbar.png'; 
	...
	this.Update = function() {
		...
		this.logoImage.style.top = this.dom.offsetTop + (this.dom.offsetHeight * 0.5 - logoImg.height * 0.5 - 100) + 'px';
		...
		this.progressFrame.style.top = this.dom.offsetTop + (this.dom.offsetHeight * 0.5 + logoImg.height * 0.5 - 90) + 'px';
